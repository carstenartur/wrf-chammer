#!/usr/bin/env python3
"""Pinned, isolated container executor for real.exe and wrf.exe steps."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one WRF step in a pinned container")
...
def inspect_image(engine: str, reference: str, identity: str) -> str:
    ...
    if not reference or not _RUNTIME_ID_RE.fullmatch(identity):
        raise ContainerExecutorError(
            "RUNTIME_IMAGE_MISMATCH", "WRF runtime reference or pinned identity is invalid."
        )
    ...
    if process.returncode != 0:
        raise ContainerExecutorError(
            "RUNTIME_IMAGE_MISMATCH", "The configured WRF runtime image is not available locally."
        )
    try:
    ...
    raise ContainerExecutorError(
        ...
        "The local WRF image does not match the immutable runtime identity.",
    )
...
def build_command(args: argparse.Namespace, specification: dict[str, Any]) -> list[str]:
    if args.step not in _SUPPORTED_STEPS:
        raise ContainerExecutorError(
            ...
            f"The WRF container executor does not implement step {args.step}.",
        )
    ...
    if specification_directory.is_symlink():
        raise ContainerExecutorError("NAMELIST_INVALID", "Immutable specification directory is a symbolic link.")
    runtime = specification["identity"].get("runtime", {}).get("wrf")
    if not isinstance(runtime, dict):
        raise ContainerExecutorError("RUNTIME_IMAGE_MISMATCH", "WRF runtime snapshot is missing.")
    reference = runtime.get("reference")
    pinned = runtime.get("identity")
    if not isinstance(reference, str) or not isinstance(pinned, str):
        raise ContainerExecutorError("RUNTIME_IMAGE_MISMATCH", "WRF runtime snapshot is invalid.")
    engine = engine_program()
    selector = inspect_image(engine, reference, pinned)
    work_name = "real" if args.step == "real" else "wrf"
    workdir = run_root / "work" / "wrf" / work_name
    ...
    command = [
        ...
        "--pids-limit",
        os.environ.get("WRF_CHAMMER_WRF_PIDS_LIMIT", "512"),
        "--tmpfs",
        "/tmp:rw,nosuid,nodev,size=512m",
    ]
    if hasattr(os, "getuid") and hasattr(os, "getgid"):
        command += ["--user", f"{os.getuid()}:{os.getgid()}"]
    cpus = os.environ.get("WRF_CHAMMER_WRF_CPUS")
    memory = os.environ.get("WRF_CHAMMER_WRF_MEMORY")
    if cpus:
        command += ["--cpus", cpus]
    if memory:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if process.returncode != 0:
            if not payload:
                write_failure(
                    args.result,
                    "PROCESS_CRASH",
                    f"WRF container exited with code {process.returncode} without a result document.",
                )
        ...
        print(f"WRF container executor failed ({type(exc).__name__}).", file=os.sys.stderr)
        return 1
