...
(async () => {
  ...
  sources[2].onerror(new Error('temporary disconnect'));
  assert.equal(sources[2].closed, false);
  assert.equal(pollingFallbacks, 0);

  sources[2].readyState = FakeEventSource.CLOSED;
  sources[2].onerror(new Error('stream unavailable'));
  assert.equal(sources[2].closed, true);
  assert.equal(pollingFallbacks, 1);
