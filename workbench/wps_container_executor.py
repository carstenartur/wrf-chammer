#!/usr/bin/env python3
"""Hardened public entry point for pinned WPS container execution."""
...
from workbench import _wps_container_executor_core as _core
from workbench._wps_container_executor_core import *  # noqa: F401,F403

_core_build_command = _core.build_command


def load_specification(directory: Path, expected_key: str) -> dict[str, Any]:
    """Load the canonical immutable specification written by the service."""

    raw_directory = _core._absolute(directory)
...
def build_command(args: Any, specification: dict[str, Any]) -> list[str]:
    """Build a Docker-compatible command using the canonical spec filename."""
